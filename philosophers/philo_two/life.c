...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/03/29 13:28:49 by sglass            #+#    #+#             */
/*   Updated: 2021/03/31 18:19:21 by sglass           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*philo_live(t_all *all)
{
	...
	while (all->inp_args->dead != 1)
	{
		sem_wait(all->inp_args->forks);
		now = my_ms(&current_time);
		ne_printf(now - all->inp_args->time_start, all->philos->number,
		" has taken a fork\n", all);
		sem_wait(all->inp_args->forks);
		now = my_ms(&current_time);
		ne_printf(now - all->inp_args->time_start, all->philos->number,
...
void	eating(t_all *all, struct timeval *current_time)
{
	...
	while (now < tmp && all->inp_args->dead != 1)
	{
		now = my_ms(current_time);
		usleep(100);
	}
