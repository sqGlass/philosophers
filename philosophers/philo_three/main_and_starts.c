/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   main_and_starts.c                                  :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: sglass <sglass@student.42.fr>              +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/03/31 15:55:15 by sglass            #+#    #+#             */
/*   Updated: 2021/04/01 10:57:10 by sglass           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "philo_header.h"

int		main(int argc, char **argv)
{
	int				ret;
	t_philos		*philos;
	t_input_args	inp_args;
	t_all			*all;

	if (argc < 5 || argc > 6)
	{
		write(1, "wrong number of args\n", 21);
		return (1);
	}
	ret = take_options(argc, argv, &inp_args);
	if (ret == 1 || inp_args.times_must_eat == 0)
		return (1);
	philos = malloc(sizeof(t_philos) * inp_args.num_of_philo);
	all = malloc(sizeof(t_all) * inp_args.num_of_philo);
	inp_args.time_start = my_ms(&inp_args.current_time);
	ret = born_semaphores(&inp_args);
	if (ret == 1)
		return (1);
	if (inp_args.times_must_eat > 0)
		pthread_create(&inp_args.countmeal, NULL, (void *)count_meal,
		&inp_args);
	born_philos(philos, &inp_args, all);
	return (0);
}

int		take_options(int argc, char **argv, t_input_args *inp_args)
{
	int i;

	i = 0;
	if (check_symbols(argc, argv) == 1)
		return (1);
	inp_args->num_of_philo = ft_atoi(argv[1]);
	if (inp_args->num_of_philo < 1 || inp_args->num_of_philo > 200)
		return (1);
	inp_args->t2d = ft_atoi(argv[2]);
	if (inp_args->t2d < 60)
		return (1);
	inp_args->t2eat = ft_atoi(argv[3]);
	if (inp_args->t2eat < 60)
		return (1);
	inp_args->t2sleep = ft_atoi(argv[4]);
	if (inp_args->t2sleep < 60)
		return (1);
	if (argc == 6)
		inp_args->times_must_eat = ft_atoi(argv[5]);
	else
		inp_args->times_must_eat = -1;
	return (0);
}

int		check_symbols(int argc, char **argv)
{
	int i;
	int j;

	i = 1;
	j = 0;
	while (i < argc)
	{
		while (argv[i][j])
		{
			if (ft_isdigit(argv[i][j]) == 0)
				return (1);
			j++;
		}
		j = 0;
		i++;
	}
	return (0);
}

int		ft_isdigit(int c)
{
	if (c >= '0' && c <= '9')
		return (1);
	return (0);
}
